...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.bind('<Escape>', lambda e: (self.canvas.delete('all'), print('*' * 20)))
        ...
        if width > dens and height > dens:
            ...
            for region in vor.regions:
                if -1 not in region and region != []:
                    ...
                    polygon = intersection(p1, p2)
                    print(polygon)
                    self.canvas.create_polygon(*polygon, outline='grey50', fill='', tags=['voronoi', 'v'])
